module.exports = {
  ...
  development : {
    ...
    OMS_LUMISECTIONS : (run_number) => `lumisections?filter[run_number]=${
                         run_number}&page[limit]=5000`,
  ...
  dev_to_prod : {
    ...
    database : process.env.DB_NAME || 'runregistry_database',
  ...
  staging : {
    username : process.env.DB_USERNAME,
    password : process.env.DB_PASSWORD,
    database : process.env.DB_NAME || 'runregistry_database_dev',
    logging : false,
  ...
  dev_kubernetes : {
    // we do not support this, so far during RUN III operation k8 cluster is
    // more buggy than VM
    username : process.env.DB_USERNAME,
    password : process.env.DB_PASSWORD,
    database : process.env.DB_NAME || 'runregistry_database_dev',
    logging : false,
    host : process.env.DB_HOSTNAME || 'dbod-gc005.cern.ch',
    dialect : 'postgres',
    port : process.env.DB_PORT || 6601,
    define : {
      // Make sequelize not pluralize the name of tables:
      freezeTableName : true,
    },
