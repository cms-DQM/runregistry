...
const requireAuth = async (req, res, next) => {
  try {
    ...
    if (email.startsWith('auto@auto')) {
      // This would be the cron job re-classifying runs when they change from
      // the API, no need to check for authentication:
      next();
      return;
    }
    ...
    // For each egroup that the user belongs to,
    // check that the requested URL matches any of the RegExps
    ...
    if (!authenticated) {
      // If it gets here, it means the user is not part of the egroup authorized
      // to perform the action they desire, therefore we will tell them which
      // e-group they need to be in to fulfill this action.

      // OR, it might happen that they are trying to perform an action which
      // requires authorization, but there is no egroup setup to handle this
      // 'route', so we will instruct for user to file a JIRA ticket.
