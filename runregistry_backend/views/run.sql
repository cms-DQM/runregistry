...
DROP AGGREGATE IF EXISTS oms_attributes (jsonb);

DROP AGGREGATE IF EXISTS rr_attributes (jsonb);

DROP AGGREGATE IF EXISTS mergejsonb (jsonb);

DROP AGGREGATE IF EXISTS mergejsonbarray (jsonb);

CREATE
OR REPLACE FUNCTION mergejsonb (jsonb, jsonb) RETURNS jsonb AS 'SELECT $1 || $2;' LANGUAGE SQL IMMUTABLE RETURNS NULL ON NULL INPUT;
...
CREATE
OR REPLACE FUNCTION mergejsonbarray (jsonb, jsonb) RETURNS jsonb AS 'SELECT array_to_json(array(select distinct jsonb_array_elements( $1 || $2)))::jsonb;' LANGUAGE SQL IMMUTABLE RETURNS NULL ON NULL INPUT;

CREATE AGGREGATE mergejsonbarray (jsonb) (
    sfunc = mergejsonbarray,
    stype = jsonb,
    initcond = '[]'
);

CREATE
OR REPLACE VIEW "RunView" AS
SELECT
    "RunEvent"."run_number",
    oms_attributes (
        "RunEvent"."oms_metadata"
        ORDER BY
            "RunEvent"."version"
    ),
    rr_attributes (
...
FROM
    "RunEvent"
    INNER JOIN "Event" ON "Event"."version" = "RunEvent"."version"
GROUP BY
    "RunEvent"."run_number";
...
CREATE
OR REPLACE VIEW "AggregatedLumisection" AS
SELECT
...
FROM
    (
        SELECT
            oms_lumisection_changes.run_number,
            mergejsonb (
                oms_jsonb
                ORDER BY
                    oms_version ASC
        ...
        FROM
            (
                ...
                SELECT
                    ...
                    offline.name,
                    jsonb AS oms_jsonb,
                    online.lumisection_number,
                    online.version AS oms_version
                FROM
                    (
                        SELECT
                            run_number,
                            ...
                            lumisection_number,
                            jsonb,
                            "OMSLumisectionEventAssignation".version
                        FROM
                        ...
                        WHERE
                            "OMSLumisectionEvent".name = 'online'
                        ORDER BY
                            "OMSLumisectionEvent".version ASC
                    ) online
                    INNER JOIN (
                        SELECT DISTINCT
                            run_number,
                        ...
                        WHERE
                            "OMSLumisectionEvent".name <> 'online'
                    ) offline ON online.run_number = offline.run_number
                ...
                FROM
                    (
                        SELECT
                            ...
                            jsonb,
                            manual_change,
                            "LumisectionEventAssignation".version AS rr_version
                        FROM
            ...
            ) rr_lumisection_changes ON oms_lumisection_changes.run_number = rr_lumisection_changes.run_number
    ...
    INNER JOIN (
        SELECT
            run_number AS dataset_run_number,
            name AS dataset_name,
            dataset_attributes
        FROM
            "Dataset"
    ) dataset ON dataset.dataset_run_number = lumisection.run_number
    AND dataset.dataset_name = lumisection.name
    -- RUN INFORMATION:
    INNER JOIN (
        SELECT
            run_number AS run_run_number,
