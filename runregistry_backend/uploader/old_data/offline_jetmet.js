...
const connectionString =
  'postgresql://fabioespinosa:@localhost:5432/intermediate_rr_3';
...
exports.save_runs_from_old_rr = async (rows, number_of_tries) => {
  ...
  const promises = rows.map(row => async () => {
    try {
      ...
      let rr_lumisections = generate_rr_lumisections(
        row,
        final_lumisections,
        preformatted_workspace_lumisections['global']
      );
      // Only interested in jetmet-jetmet and jetmet-jetmet_private
      rr_lumisections = rr_lumisections.map(lumisection => {
        return {
          'jetmet-jetmet': lumisection['jetmet-jetmet'],
          'jetmet-jetmet_private': lumisection['jetmet-jetmet_private']
        };
      });
      ...
      dataset_attributes = {
        jetmet_state: dataset_attributes['jetmet_state']
      };
      await axios.post(
        `${API_URL}/datasets`,
        {
